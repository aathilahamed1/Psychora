...
export default function PeerSupportPage() {
  ...
  const handleReport = (postId: number) => {
    ...
    toast({
        title: "Post Reported",
        description: "Thank you for helping keep the community safe. A moderator will review this post.",
    })
  }
                    ...
                    <div className="flex items-start justify-between">
                        <div>
                         ...
                         <div className="flex items-center gap-1">
                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleReport(post.id)} aria-label="Report post">
                                <Flag className={cn("h-4 w-4", post.reported ? 'text-destructive fill-destructive' : 'text-muted-foreground')} />
                            </Button>
                            {canModerate && (
                                <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-destructive" onClick={() => handleDelete(post.id)} aria-label="Delete post">
